...
knit: (function(inputFile, encoding) {
  ...
  rmarkdown::render(inputFile,
    encoding=encoding,
    output_file=file.path(dirname(inputFile), out_dir, 'genchi-executive-report_en.pdf')
    )
  })
---
...
```{r 3_cleaning1|comparison, eval=FALSE, include=FALSE, include=FALSE}
...
```{r 3_cleaning4|comparison, eval=FALSE, include=FALSE, include=FALSE}
...
```{r _translate|reduce_df, include=FALSE}
...
```{r _translate|freq_one-word_translation, include=FALSE}
...
```{r _translate|TF-IDF_one-word_plotting_translated1, message=FALSE, include=FALSE}
...
```{r _translate|TF-IDF_one-word_plotting_translated2, message=FALSE, echo=FALSE}
one_word_plot_en
```
...
We calculate and plot the most frequent bigrams for each speech to perform a TF-IDF analysis then.  This analysis allows to determine how important is each bigram from a document in relation to a set of documents.  In this case, we can answer the question: what two-words concepts are most important in a speech with respect to the rest?
...
```{r _translate|freq_two-word_translation, include=FALSE}

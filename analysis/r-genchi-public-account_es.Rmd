...
author: "Autores: Fabián Álvarez / Roberto Rodríguez - Actualización: Fabián Álvarez"
date: "Primera edición: 09-11-2020 - Actualización: 08/05/2022"
...
```{r cleaning1|additional_corrections}
speech_2021 <- speech_2021 %>% 
...
speech_2018 <- speech_2018 %>% 
  ...
  str_remove_all("\\s+PROGRAMApdf_documentS\\s+RE.+LACTANTES") %>% 
...
En el discurso del 2018 se dio énfasis a la actividad, capacitación y colocación *laboral*, y a aquellos *penados* que han sido intervenidos.  Asimismo, se rindió cuenta de *recursos* ejecutados durante el 2017, financiados por Ley de Presupuesto.  También se menciona la impartición de *diplomados* para el perfeccionamiento del personal.
...
Vemos que el término *covid* se tomó la atención del discurso 2020, lo cual tiene mucho sentido en atención a la emergencia *sanitaria* de pandemia que comenzó en dicho año y el consecuente riesgo de *contagio*.  Por otro lado, se informa sobre la publicación de un estudio de reincidencia que consideró *egresos* de privados de libertad en el año 2016.  Además, se puso atención a la gran cantidad de detenidos durante el 2019 que fueron recibidos con *lesiones* evidentes desde las instituciones policiales, para lo cual se aprobaron protocolos de constatación de estado de salud.

Por último, el discurso del 2021 enfatizó dos grandes temáticas.  La primera, relacionada con la salud *mental* y la derivación de posibles *pacientes* privados de libertad con necesidades de atención *clínica* psicológica en el ámbito de la *psicoterapia* individual.  Esto, en atención especialmente a que durante el 2020 se mantuvo el riesgo sanitario de pandemia.  El segundo tópico abordó el desarrollo de una Cuenta Pública *Participativa* con mayor inclusión social y con transmisión vía streaming por segundo año consecutivo.


# Análisis por bigramas

Definimos algunas *stopwords* para descartar adicionalmente a las ya definidas y calculamos la frecuencia de los bigramas para cada documento.

```{r 5_two-words_analysis|more_stopwords}
bi_stopwords <- tibble(palabra = c("cuenta", "pública")) 
```

# Frecuencias

```{r 5_two-words_analysis|bigram_frequencies_2021}
...
```{r 5_two-words_analysis|bigram_frequencies_2020}
...
```{r 5_two-words_analysis|bigram_frequencies_2019}
...
```{r 5_two-words_analysis|bigram_frequencies_2018}
